...
class FlowNets(nn.Module):
    def __init__(self, in_planes=6, batchNorm=False):
        ...
        self.upsample_flow6to5 = nn.ConvTranspose2d(2, 2, 4, 2, 0, bias=False)
        self.upsample_flow5to4 = nn.ConvTranspose2d(2, 2, 4, 2, 0, bias=False)
        self.upsample_flow4to3 = nn.ConvTranspose2d(2, 2, 4, 2, 0, bias=False)
        self.upsample_flow3to2 = nn.ConvTranspose2d(2, 2, 4, 2, 0, bias=False)
    ...
    def forward(self, x):
        ...
        flow6_up = self.antipad(self.upsample_flow6to5(flow6),
        ...
        flow5_up = self.antipad(self.upsample_flow5to4(flow5),
        ...
        flow4_up = self.antipad(self.upsample_flow4to3(flow4),
        ...
        flow3_up = self.antipad(self.upsample_flow3to2(flow3),
    ...
    def conv(self, in_planes, out_planes, batchNorm, kernel_size=3, stride=1, padding=1):
        if batchNorm:
            return nn.Sequential(
                ...
                nn.BatchNorm2d(out_planes), nn.LeakyReLU(0.1, inplace=True))
        else:
            return nn.Sequential(
...
if __name__ == "__main__":
    ...
    data = torch.randn(1, 6, 1024, 2048).cuda()
    with torch.no_grad():
        out = model(data)
        print(out.shape)
