...
class camvid_video_dataset(Dataset):
    ...
    def get_list(self, list_path):
        ...
        for i, line in enumerate(lines):
            ...
            subdir = gt_label_name.split('_')[0]
            img_3_id = int(img_3_name[:-4].split('_')[-1])

            for j in range(2, 11):
                img_1_id = img_3_id - j
                img_2_id = img_3_id - random.randint(1, j - 1)
                img_1_name = img_3_name.replace('{:06d}.png'.format(img_3_id), '{:06d}.png'.format(img_1_id))
                img_2_name = img_3_name.replace('{:06d}.png'.format(img_3_id), '{:06d}.png'.format(img_2_id))
...
class camvid_video_dataset_PDA(Dataset):
    ...
    def get_list(self, list_path):
        ...
        for i, line in enumerate(lines):
            ...
            self.gt_label_name.append(gt_label_name)
            img_id = int(name[-10:-4])
            img_name_list = []
            for j in range(10):
                img_name = name.replace('{:06d}.png'.format(img_id), '{:06d}.png'.format(img_id - 9 + j))
...
if __name__ == '__main__':
    ...
    list_path = '/ghome/zhuangjf/git_repo/DAVSS/data/list/camvid/trainval.txt'
    gt_path = '/gdata/zhuangjf/CamVid'
    batch_size = 1
    shuffle = False

    train_data = camvid_video_dataset(data_path, gt_path, list_path, crop_size=None)
    ...
    # ###############################################################################################################################
    # data_path = '/gdata/zhuangjf/CamVid'
    # list_path = '/ghome/zhuangjf/git_repo/DAVSS/data/list/camvid/test.txt'
    # gt_path = '/gdata/zhuangjf/CamVid'
    # batch_size = 1
    # shuffle = False

    # val_data = camvid_video_dataset_PDA(data_path, gt_path, list_path)
