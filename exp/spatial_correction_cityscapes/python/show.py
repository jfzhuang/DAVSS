...
from lib.dataset.utils import decode_labels
    ...
    with torch.no_grad():
        for d in range(9, distance):
            for step, sample in enumerate(test_data_loader):
                ...
                feat_rgb = cv2.resize(feat_rgb, (512, 256))
                ...
                for i in range(d):
                    ...
                    feat_merge_rgb = cv2.resize(feat_merge_rgb, (512, 256))
                    result_list.append(feat_merge_rgb)

                gt_rgb = gt_label.clone()
                gt_rgb = decode_labels(gt_rgb.squeeze())
                gt_rgb = cv2.resize(gt_rgb, (512, 256))
                result_list.append(gt_rgb)

                zeros = np.zeros([256, 512, 3], dtype=np.uint8)
